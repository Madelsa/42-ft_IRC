...
/*   Updated: 2024/10/21 14:43:27 by mabdelsa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Client::handleCapCommand(Client *client, const std::vector<std::string> &params) {
    ...
    } else if (client->getRegisterSteps(0)) { // Check if the client is in the appropriate registration step
        // Handle capability requests
        if (params.size() == 1 && params[0] == "REQ") {
            client->getServerReplies().push_back(":ircserv CAP * REQ : \r\n"); // Acknowledge capability request
        } else if (params.size() == 1 && params[0] == "NAK") {
            client->getServerReplies().push_back(":ircserv CAP * NAK : \r\n"); // Notify about capability denial
        } else if (params.size() == 1 && params[0] == "ACK") {
            client->getServerReplies().push_back(":ircserv CAP * ACK : \r\n"); // Acknowledge received capabilities
        }
    }
}
